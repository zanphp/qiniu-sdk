...
final class Response
{
    ...
    {
        // 兼容swoole_http_client
        $headers = array_combine(array_map("strtolower", array_keys($this->headers)), array_values($this->headers));
        $via = $headers[strtolower('X-Via')];
        if ($via === null) {
            $via = $headers[strtolower('X-Px')];
        }
        if ($via === null) {
            $via = $headers[strtolower('Fw-Via')];
        }
        return $via;
    }
    ...
    public function xLog()
    {
        // 兼容swoole_http_client
        $headers = array_combine(array_map("strtolower", array_keys($this->headers)), array_values($this->headers));
        return $headers[strtolower('X-Log')];
    }
    ...
    public function xReqId()
    {
        // 兼容swoole_http_client
        $headers = array_combine(array_map("strtolower", array_keys($this->headers)), array_values($this->headers));
        return $headers[strtolower('X-Reqid')];
    }
    ...
    private static function isJson($headers)
    {
        ...
        return array_key_exists(strtolower('Content-Type'), $headers) &&
        strpos($headers[strtolower('Content-Type')], 'application/json') === 0;
    }
}
